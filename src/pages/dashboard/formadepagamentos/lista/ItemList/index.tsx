...
const ItemList = () => {
  ...
  const searchAndGetCollection = async () => {
    ...
    if (search !== '') {
      queryParams = `${baseUrlApi}/formasdepagamento?nomeCliente_clientes=${search}&limit=${limitRows}`
    } else {
      queryParams = `${baseUrlApi}/formasdepagamento?limit=${limitRows}`
    }
  ...
  const handleUrl = item => {
    if (item) {
      router.push({
        pathname: '/dashboard/formadepagamentos/edit',
  ...
  const deleteItem = item => {
    if (confirm('Tem certeza que deseja excluir este item?') == true) {
      fetch(`${baseUrlApi}/formasdepagamento/${item}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json'
        }
        ...
        <Grid container>
          ...
          <Grid item xs={12} md={12} lg={2}>
            ...
            <Button
              ...
              color='secondary'
              onClick={() => {
                router.push('/dashboard/formadepagamentos/')
              }}
            >
              Nova F.: Pagamento
            </Button>
        ...
        <Table sx={{ minWidth: 650 }} aria-label='simple table'>
          ...
          {data.length > 0 && (
            <TableBody>
              {data.map((item, index) => (
                <TableRow
                  key={item.id_FormaDePagamento_formasDePagamentos}
                  sx={{
                ...
                >
                  <TableCell component='th' scope='row'>
                    {item.id_FormaDePagamento_formasDePagamentos}
                  </TableCell>
                  <TableCell align='left'>{item.nomeFormaDePagamento_formasDePagamentos}</TableCell>
                  <TableCell align='left'>{item.created_at}</TableCell>
                  <TableCell align='left'>
                    <Button variant='contained' onClick={() => handleUrl(item.id_FormaDePagamento_formasDePagamentos)}>
                      EDITAR
                    </Button>
                    <Button
                      sx={{ marginLeft: 3 }}
                      variant='contained'
                      onClick={() => deleteItem(item.id_FormaDePagamento_formasDePagamentos)}
                    >
                      EXCLUIR
                    </Button>
                  </TableCell>
