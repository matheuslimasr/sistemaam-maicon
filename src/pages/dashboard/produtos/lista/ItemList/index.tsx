...
const ItemList = () => {
  ...
  const searchAndGetCollection = async () => {
    ...
    if (search !== '') {
      queryParams = `${baseUrlApi}/produtos?nomeProduto_produtos=${search}&limit=${limitRows}`
    } else {
      queryParams = `${baseUrlApi}/produtos?limit=${limitRows}`
    }
  ...
  const deleteItem = item => {
    if (confirm('Tem certeza que deseja excluir este item?') == true) {
      fetch(`${baseUrlApi}/produtos/${item}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json'
        }
  ...
  const handleUrl = item => {
    if (item) {
      router.push({
        pathname: '/dashboard/produtos/edit',
        query: { uid: item }
      })
    }
  }
              ...
              {data.map((item, index) => {
                return (
                  ...
                  >
                    ...
                    <TableCell align='left'>{item.nomeProduto_produtos}</TableCell>
                    <TableCell align='left'>{item.referenciaProduto_produtos}</TableCell>
                    <TableCell align='left'>{item.created_at}</TableCell>
                    <TableCell align='right'>
                      <Button variant='contained' onClick={() => handleUrl(item.id_produtos)}>
                        EDITAR
                      </Button>
                      <Button sx={{ marginLeft: 3 }} variant='contained' onClick={() => deleteItem(item.id_produtos)}>
                        EXCLUIR
                      </Button>
                    </TableCell>
                  </TableRow>
